...
// dependencies. Ported verbatim from WEAKNIGHT_BEDROCK_RACERS' own copy
...
// authenticates via POST /api/v1/auth/email/login; PAPERCRAFT's own
// single-node game server (apps/server) verifies the HMAC locally with a
// shared secret (see papercraft_protocol.h's own PC_TICKET_* constants) --
// no asymmetric crypto, no blocking network call per connect.
#ifndef PAPERCRAFT_HMAC_SHA256_H
#define PAPERCRAFT_HMAC_SHA256_H
